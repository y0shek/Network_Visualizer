...
for(var i=1; i < 51; i++){
  nodes.push({
    id          : i,
    coords      : [(Math.random()*.2)+42.98, (Math.random()*.5)-89.65],
...
var originRadius //for holding original radius value
...
function mouseOver(d) {
  ...
  d3.select(this).transition()
    .duration(100)
    .attr("r", function(){
      originRadius = d.radius;
      d.radius = look_radius;
      return d.radius;
    })
    .attr("stroke-width", look_radius/8+1);

  // get group object and save it in var parent
  var parent = d3.select(this.parentNode);

  // enlargen and UPDATE images from dir
  parent.select("pattern").select("image").transition()
    .duration(100)
  ...
  parent.select("text.names").transition()
    .duration(100)
    .attr("font-size", d.radius/6 +12)
    .attr("transform", function(d){
    return "translate(0,"+ (originRadius - d.radius-5) +")"
    })
    ;
  parent.select("text.cities").transition()
    .duration(100)
    .attr("font-size", look_radius/6+8)
        .attr("transform", function(d){
    return "translate(0,"+ (d.radius-originRadius+5) +")"
    })
    ;
    force.resume();
}
...
function mouseOut(d) {
  ...
  d3.select(this).transition()
    .duration(1000)
    .attr("r", function(){
      d.radius = originRadius;
      return d.radius;
    })
    .attr("stroke-width", d.radius/8+1);
...
//ADD THE GROUP TO BE CENTERED IN SVG
var grp = svg_elem
  .append("g")
  .attr("class", "groups")
  .attr("transform", function(d){return "translate("+d.radius*2+","+d.radius*2+")";})
  .on("mousedown", function(d){
    d3.select(this).select("circle").transition().duration(100)
    .attr("transform", "translate(0,"+ (-d.radius/2) +")");
  })
...
var names = grp
  ...
  .attr("x", 0)
  .attr("y", function(d){return (-d.radius*1.25);})
  .attr("text-anchor", "middle")
  .attr("font-family", "sans-serif")
  .attr("font-size", function(d){
    return d.radius/6 +8;
...
var cities = grp
    ...
    .attr("x", 0)
    .attr("y", function(d){return d.radius*1.4;})
    .attr("text-anchor", "middle")
    .attr("font-family", "sans-serif")
    .attr("font-size", function(d){
      return d.radius/6 +8;
...
//OPENLAYERS INTEGRATION STARTS HERE
/*
var map = new ol.Map({
  layers: [
    new ol.layer.Tile({
